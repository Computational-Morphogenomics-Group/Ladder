...
class CSVAE(nn.Module):
    ...
    # Define Params
    def __init__(self, input_size, label_size, common_latent_size=2, weighted_latent_size=2, mlp_hidden=64, mlp_hidden_count=3, betas=[20,1,0.2,10,1]):
        ...
        self.x_latent = MLP(input_size=self.z_size + self.w_size, hidden_sizes=[self.z_size+self.w_size]*mlp_hidden_count, output_size=self.z_size+self.w_size)
        self.mu_x = MLP(input_size=self.z_size + self.w_size, hidden_sizes=[mlp_hidden]*mlp_hidden_count, output_size=self.x_size)
        self.logvar_x = MLP(input_size=self.z_size + self.w_size, hidden_sizes=[mlp_hidden]*mlp_hidden_count, output_size=self.x_size)
    ...
    def generate(self, x, y, hard_latent=False):
        ...
        w_prior_mu, w_prior_var = self.w_prior(y)
        w = self.reparam(w_prior_mu, w_prior_var, var_log = False)
