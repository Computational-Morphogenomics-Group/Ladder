...
GAMMA = np.round((1.0 - math.gamma(1 + 1.0e-8)) * 1.0e14) * 1.0e-6
...
def _get_sliced_wasserstein_n_to_1(
    ...
    unif_simplex_a, unif_simplex_b = torch.ones(orig.shape[0]).div(
        orig.shape[0]
...
def self_profile_reproduction(
    ...
    samples = torch.stack(
        [
            point_set[p_simplex.multinomial(samp_size).tolist()]
